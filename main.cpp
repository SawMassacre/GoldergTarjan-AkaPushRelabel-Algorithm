...
int runTests(int testNum) {
    ...
    //g.addEdge надо вынести отдельной функцией в самый конец кода чтобы хранить там наборы
    //инициализации графов, в тестах просто вызывать эту функцию и функцию гольдберга
    //тарьяна параллельно с алгоритмом диница, замерить время работы
    switch (c) {
        case 0: {
            V = 6;
            Graph g(V);
